...
function apt -d "Smart wrapper routing apt commands to paru/yay/pacman"
    ...
    switch $action
        ...
        case search
            ...
            # 搜索不需要 sudo，直接用 pkg_mgr 不带 sudo 前缀
            $pkg_mgr -Ss $argv
        case show
        ...
        case autoremove
            ...
            if test (count $orphans) -gt 0
                if test $is_zh -eq 1
                    echo "找到 "(count $orphans)" 个孤立的软件包。正在逐个通过 $pkg_mgr 卸载..."
                else
                    echo "Found "(count $orphans)" orphaned package(s). Removing one by one via $pkg_mgr..."
                end
                for pkg in $orphans
                    $cmd -Rns $pkg
                end
