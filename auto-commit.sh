...
# =============================================================================
# 由 systemd user timer dotfiles-autocommit.timer 每 3 天凌晨调用
# 也可以手动运行: bash ~/dotfiles/auto-commit.sh
#
# 逻辑:
#   1. 检查工作区是否干净（git status --porcelain）
#   2. 有变更:
#      a. 生成 commit message（包含改动文件摘要 + 时间戳）
#      b. git add -A（.gitignore 已兜底排除所有不该跟踪的）
#      c. git commit
#      d. git pull --rebase（避免远程有新 commit 时 push 被拒）
#      e. git push
#   3. 无变更: 静默退出
#
...
# 静默 git 调用（在 ~/dotfiles 里操作，普通 git）
g() { git "$@"; }
...
# git add -A 安全性说明:
#   .gitignore 用黑名单模式排除敏感文件（.ssh/ .gnupg/ .env *.key 等）
#   仓库根目录在 ~/dotfiles，只跟踪仓库内的文件，不会误加 $HOME 下的东西
g add -A
...
if echo "$pull_output" | grep -qiE "conflict|失败|error|fatal|couldn't|unable"; then
    log "⚠ pull --rebase 失败或有冲突，跳过 push（请手动处理: cd ~/dotfiles && git status / git rebase --abort）"
    exit 1
fi
